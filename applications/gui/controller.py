...
def doMoves(states):
  ...
  try:
    if states['shift']:
        sw3.RelativeDepth(-rise_speed).start()
    elif states[' ']:
        sw3.RelativeDepth(rise_speed).start()
    else:
      #neither shift or space being pressed, so depth should stay the same
      sw3.RelativeDepth(0).start()
  except:
    # If this throws exception, the robot is trying to move up above the water
    pass
  # if left ctrl being held, 'a' and 'd' cause robot to strafe
  if states['l-ctrl']:
    if states['a']:
        sw3.Strafe(-speed).start()
    elif states['d']:
        sw3.Strafe(speed).start()
    else:
        ...
        sw3.Strafe( 0 ).start()
  # if left ctrl not being held, 'a' and 'd' cause robot to turn
  else:
    if states['a']:
