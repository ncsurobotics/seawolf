...
int main(int argc, char *argv[]) 
{
	...
	for(j=0; j<4; j++)
	{
		channel[j] = (signed short*)calloc(262144,sizeof(signed short));
	}
...
void OpenData(char * filename)
{
	...
	for( i=0; i<262144; i++)
	{
		...
		fscanf(fp, "%f", &buffer);
		channel[0][i] = (signed short)buffer;
...
void DrawData(int start)
{
	...
	glBegin(GL_LINES);
		...
		for(i=start; i<(262144-1) && i<(start+WINDOW_X*XRES); i+=XRES)
		{
					
			glVertex3f((float)(i-start)/(float)XRES, ((float)WINDOW_Y/4.0)+(((float)channel[0][i]/(float)max)*100), 0.0f);
			i+=XRES;
			glVertex3f((float)(i-start+1)/(float)XRES, ((float)WINDOW_Y/4.0)+(((float)channel[0][i]/(float)max)*100), 10.0f);
			
			//printf("\n%d\t%d\t%d\n\t%f",start,channel[0][i],max,  ((float)WINDOW_Y/4.0)+(((float)channel[0][i]/(float)max)*100)  );
		}
