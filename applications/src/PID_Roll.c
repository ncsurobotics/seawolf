...
int main(void) {
    ...
    // Pull PID constants
    K_p = SeaSQL_getRollPID_p();
    K_i = SeaSQL_getRollPID_i();
    K_d = SeaSQL_getRollPID_d();
...
// Get the roll heading
float getRollHeading(void) {
    // Desired roll is always 0
    return 0.0;
}

// Get current roll
float getRoll(void) {
    // Get the stabalized euler angles for roll
    return SeaSQL_getSEA_Roll();
}

// Spit out some data
void dataOut(float MV){
    int y = (int) MV;
    Notify_send("THRUSTER_REQUEST", Util_format("Roll %d %d", -y, y));
}
