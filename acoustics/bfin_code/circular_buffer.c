        ...
        while(state != DONE) {
            ...
            if(state == READING && cirbuff_full) {
				...
				*/
				...
				printf("Looking for trigger... %hd \n", (short)*(cirbuff[A]+cirbuff_offset));
				memcpy( fftInput, cirbuff[A] + cirbuff_offset, sizeof(adcsample) * SAMPLES_PER_CHANNEL );
				printf("Got data %hd \n");
				/*
				for ( i = 0 ; i < SAMPLES_PER_CHANNEL ; i++ )
				{
					fftInput[i] = *(cirbuff[A] + cirbuff_offset + i);
					printf("capture: %d \n", i);
				}
				*/
				//Zip data through FFT
				fft(fftInput, fftOutput, SAMPLES_PER_CHANNEL);
				printCSVcmplx(fftOutput, SAMPLES_PER_CHANNEL);	
				//Threshold frequency value
				
				if( (short)fftOutput[UN_FREQ].re >= THRESH_LEVEL )
				{
