...
class BuoysEntity(VisionEntity):
    ...
    def mouse_callback(self, event, x, y, flags, _):
        if event == cv.CV_EVENT_LBUTTONDOWN:
            self.trackers = [
                libvision.Tracker(self.frame, (x,y), (50,50), (150,150), debug=False)
            ]
            self.state = "tracking"
    ...
    def initialize_non_pickleable(self, debug=True):
        ...
        if debug:
            cv.SetMouseCallback("BuoysEntity", self.mouse_callback, None)
    ...
    def find(self, frame, debug=True):
        ...
        scale = 0.7
        frame = scale_out_of_place(frame, (frame.width*scale, frame.height*scale))
...
def scale_in_place(image, new_size):
    ...
    '''
    copy = cv.CreateImage(cv.GetSize(image), 8, image.channels)
    cv.Copy(image, copy)
    cv.SetImageROI(image, (0, 0, new_size[0], new_size[1]))
    cv.Resize(copy, image, cv.CV_INTER_NN)

def scale_out_of_place(image, new_size):
    new_image = cv.CreateImage(new_size, 8, image.channels)
    cv.Resize(image, new_image, cv.CV_INTER_NN)
    return new_image
