...
IplImage* find_target_color(IplImage* in, int t1, int t2, int t3, int min_blobsize, int dev_threshold, double precision_threshold){ //should find the set of colors closest to the target color
    int i,j,s;
    ...
    IplImage* out = cvCreateImage(cvGetSize(in),8,3);
    uchar* ptrIn = (uchar*) in->imageData;
    uchar* ptrOut = (uchar*) out->imageData;
    RGBPixel imgAverage;
    RGBPixel tempPixel;

    //this used to be passed as an argument.  Altered to make it work better with python
    RGBPixel color; 
        color.r = t1;
	color.g = t2;
	color.b = t3;
