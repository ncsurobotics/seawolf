...
class BinsMission(MissionBase):
    ...
    def sweep(self, bins):
        ...
        turning = sw3.CompoundRoutine(sw3.Forward(0,1), sw3.RelativeYaw(180))
        ...
        if current_bin:
            if current_bin.shape is "A" or current_bin.shape is "B":
                if self.dropped is "A" and current_bin.shape is "B":
                    self.nextState()
                if self.dropped is "B" and current_bin.shape is "A":
                    self.nextState()
